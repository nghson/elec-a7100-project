...
/* Add a driver to the database (A).
...
Driver *add_driver(Driver *list, unsigned long size, char *command) {
    ...
    Driver *new_list = realloc(list, ((size)+1)*sizeof(Driver));
...
/* Update time to driver (U).
 * Return index of the updated driver. */
...
int compare_time(const void *a, const void *b) {
    const Driver *s = (const Driver *)a; 	//first compare hour, then minute, then second
    const Driver *t = (const Driver *)b;
    if (s->hour != t->hour)
...
Driver *load_results(const char *command, unsigned long *size) {
    ...
    Driver *list = malloc(sizeof(Driver));
    unsigned long i = 0; 				//size of database
    int j = 0, n = 0; 					//counter for string
    char line[81]; 					

    while (fgets(line, 81, f)) {
	list = realloc(list, (i+1)*sizeof(Driver)); 	//increase database
...
int execute(Data *data, char *command) {
    ...
    switch(command[0]) {
	case 'A':
	    data->list = add_driver(data->list, data->size, command);
	    if (!command[0]) {
		return -1;
	    }
	    printf("Successfully added driver %s of team %s.\n", data->list[data->size].name, data->list[data->size].team);
	    data->size++;
	    data->sort = 1;
	    return 0;
	...
	case 'L':
	    if (data->sort) {
		qsort(data->list, data->size, sizeof(Driver), compare_time);
		data->sort = 0;
	    }
...
int main() {
    ...
    Data data;
    data.size = 0;
    data.list = malloc(sizeof(Driver));
    data.sort = 0;
    int end = 0;
